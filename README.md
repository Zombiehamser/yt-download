Отлично, я полностью прочитал все четыре файла. Теперь проведу детальное сравнение.

**Найденные расхождения README vs скрипт v5.1:**

1. **Версия**: В обоих README указано `3.0` — должно быть `5.1`
2. **Имя скрипта**: `yt-download3_EN.py` / `yt-download3_RU.py` → `yt-download5.1_EN.py` / `yt-download5.1_RU.py` (в Quick Start и Usage)
3. **Незакрытый code fence** в секции Quick Start (отсутствует ` ``` ` после команды запуска)
4. **Поддержка плейлистов** — скрипт имеет полноценный функционал (`is_playlist_url`, `get_playlist_info`, подпапки), в README отсутствует
5. **Папка `downloads/`** — скрипт сохраняет всё в подпапку `downloads/`, плейлисты в `downloads/<playlist>/`, README показывает файлы в корне
6. **Таймаут**: README везде указывает 60 минут, скрипт использует `7200 if is_playlist else 3600` (120 мин для плейлиста / 60 мин для одиночного видео)
7. **Параметры конфигурации**: в README отсутствуют `--extractor-retries '8'` и `--file-access-retries '5'`, а также `--remote-components ejs:github`
8. **Шаблон имени файла**: README показывает один статичный шаблон, скрипт использует динамический (отдельный для плейлиста и одиночного видео)
9. **FAQ о качестве**: указан несуществующий форматный ключ `bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best` — в скрипте просто `-f bestvideo+bestaudio/best` + `--merge-output-format mp4`
10. **Логика работы**: в README отсутствует шаг проверки прогресса плейлиста

***

Ниже — полные исправленные версии обоих документов:
